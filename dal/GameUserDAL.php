    ...
    public function saveContestantGame($uid, $game, $console) {
        ...
        try {
			...
			if($contestant_id==$uid){
				 $statement = $conn->prepare("UPDATE tb_game_contestant SET game_id = ?, console_id = ?");
            	$statement->bindParam(1, $game);
            	$statement->bindParam(2, $contestant);
				$statement->execute();
				}
			else{
				 $statement = $conn->prepare("INSERT INTO tb_game_contestant (tb_game_contestant, game_id, console_id) values (?, ?, ?)");
           		 $statement->bindParam(1, $uid);
